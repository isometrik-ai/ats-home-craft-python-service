...
async def create_session(session_data: Dict[str, Any], organization_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.error("Supabase API error creating session: %s", e, exc_info=True)
        raise
    except (HTTPError, RequestError, TimeoutException) as e:
        logger.error("Network error creating session: %s", e, exc_info=True)
        raise
    except (KeyError, TypeError, ValueError) as e:
        logger.error("Data validation error creating session: %s", e, exc_info=True)
        raise
...
async def update_session(session_id: str, organization_id: str,
    ...
    try:
        ...
        logger.error("Supabase API error updating session: %s", e, exc_info=True)
        raise
    except (HTTPError, RequestError, TimeoutException) as e:
        logger.error("Network error updating session: %s", e, exc_info=True)
        raise
    except (KeyError, TypeError, ValueError) as e:
        logger.error("Data validation error updating session: %s", e, exc_info=True)
        raise
...
async def check_session_exists(session_id: str, organization_id: str) -> bool:
    ...
    try:
        ...
        logger.error("Supabase API error checking session exists: %s", e, exc_info=True)
        raise
    except (HTTPError, RequestError, TimeoutException) as e:
        logger.error("Network error checking session exists: %s", e, exc_info=True)
        raise
    except (KeyError, TypeError, ValueError) as e:
        logger.error("Data validation error checking session exists: %s", e, exc_info=True)
        raise
...
async def get_sessions_list(organization_id: str, filters: SessionFilter) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        logger.error("Supabase API error getting sessions list: %s", e, exc_info=True)
        raise
    except (HTTPError, RequestError, TimeoutException) as e:
        logger.error("Network error getting sessions list: %s", e, exc_info=True)
        raise
    except (KeyError, TypeError, ValueError) as e:
        logger.error("Data validation error getting sessions list: %s", e, exc_info=True)
        raise
...
async def get_sessions_count(organization_id: str, filters: SessionFilter) -> int:
    ...
    try:
        ...
        logger.error("Supabase API error getting sessions count: %s", e, exc_info=True)
        raise
    except (HTTPError, RequestError, TimeoutException) as e:
        logger.error("Network error getting sessions count: %s", e, exc_info=True)
        raise
    except (KeyError, TypeError, ValueError) as e:
        logger.error("Data validation error getting sessions count: %s", e, exc_info=True)
        raise
