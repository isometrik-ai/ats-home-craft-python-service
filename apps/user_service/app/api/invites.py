...
def _validate_invite_request(request: InviteCreateRequest) -> None:
    ...
    if not validate_role(request.role):
        handle_invite_validation_error(
            "role", request.role, "Invalid role. Must be owner, admin, or member")
...
async def create_invitation(
    ...
    await require_permission(
        permission_code=SETTINGS_USERS_MANAGE,
    ...
    # Validate organization access
    if not validate_organization_access(user_context, organization_id):
...
async def revoke_invitation(
    ...
    # Validate organization access
    if not validate_organization_access(user_context, invitation_data["organization_id"]):
