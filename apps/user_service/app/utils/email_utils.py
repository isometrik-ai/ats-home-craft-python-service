...
UNIT_ASSIGNMENT_WELCOME_TEMPLATE_KEY = "unit_assignment_welcome"
UNIT_ASSIGNMENT_WELCOME_TRIGGER_NAME = "unit_assignment_welcome"
...
def _build_unit_assignment_welcome_context(
    *,
    ...
    return {
        ...
        "ios_app_store_url": _normalize_store_url(ios_app_store_url),
        "android_play_store_url": _normalize_store_url(android_play_store_url),
        "current_year": datetime.now().year,
...
async def resolve_unit_assignment_welcome_content(
    ...
    return render_transactional_email(UNIT_ASSIGNMENT_WELCOME_TEMPLATE_KEY, context)
...
def send_unit_assignment_welcome_email(
    *,
    ...
    try:
        context = _build_unit_assignment_welcome_context(
        ...
        subject, message, html_message = render_transactional_email(
...
async def send_unit_assignment_welcome_email_for_org(
    *,
    db_connection: asyncpg.Connection,
    organization_id: str,
    email: str,
    ...
    """Send unit welcome email using org DB template when published, else file templates."""
    try:
        context = _build_unit_assignment_welcome_context(
            first_name=first_name,
            organization_name=organization_name,
        ...
        subject, message, html_message = await resolve_unit_assignment_welcome_content(
            db_connection,
            organization_id,
            context,
        )
