...
class ActivityService:
    ...
    async def get_company_activity(
        ...
        *,
        ...
        """Get flattened activity items for a company id.

        Mirrors `get_contact_activity` behavior:
        - `limit` / `offset` paginate audit log rows in SQL (newest first)
        - returned `items` are flattened lines, often one per changed field
        - `total` is the number of audit log rows for this company
        """
        rows, total = await self.audit_log_repository.get_activity_logs_for_record_with_actor_names(
            organization_id=self.user_context.organization_id,
    ...
    def _get_display_values_for_company_field(
        self,
        *,
        field_path: str,
        audit_row: _AuditRow,
    ) -> tuple[str | None, str | None]:
        """Return optional human-friendly display values for special company fields."""
        field_key = field_path.split(".")[-1]

        # If company audit snapshots embed associated contacts, prefer a formatted label.
        # This is best-effort and does not perform DB lookups.
        if field_key in {"contacts", "contact_association"}:
            old_values_blob = _coerce_audit_values_blob(audit_row.old_values)
            new_values_blob = _coerce_audit_values_blob(audit_row.new_values)
            old_name = self._format_association_names(
                old_values_blob,
