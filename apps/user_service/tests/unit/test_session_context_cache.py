...
async def test_resolve_session_context_uses_redis_before_db():
    ...
    with (
        ...
        patch(
            "libs.shared_utils.session_context_cache._is_session_revoked",
            new=AsyncMock(return_value=False),
        ),
        patch(
            "libs.shared_utils.session_context_cache._get_redis_session_context",
            new=AsyncMock(return_value={"organization_id": "org-redis"}),
        ),
    ):
