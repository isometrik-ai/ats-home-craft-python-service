...
@pytest.mark.asyncio
async def test_list_attendance_absence_dates_unit_scoped_query():
    conn = _FakeConn(rows=[{"attendance_date": "2026-05-10"}])
    repo = DailyHelpRepository(db_connection=conn)
    ...
    assert "unit_id = $3::uuid" in query
    assert args == (ORG, PROFILE, UNIT, 2026, 5)
