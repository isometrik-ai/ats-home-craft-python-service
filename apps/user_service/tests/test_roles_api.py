...
class TestGetRoles:
    ...
    def test_list_roles_with_filters(self, client):
        ...
        with patch("apps.user_service.app.api.admin_management.roles.get_roles_list", AsyncMock(return_value=[])), \
             patch("apps.user_service.app.api.admin_management.roles.get_roles_count", AsyncMock(return_value=0)):
    ...
    def test_list_roles_database_error(self, client):
        ...
        with patch("apps.user_service.app.api.admin_management.roles.get_roles_list", AsyncMock(side_effect=Exception("Database error"))), \
             patch("apps.user_service.app.api.admin_management.roles.get_roles_count", AsyncMock(side_effect=Exception("Database error"))):
...
class TestGetRoleById:
    ...
    def test_get_role_by_id_not_found(self, client):
        ...
        role_id = str(uuid.uuid4())
        with patch("apps.user_service.app.api.admin_management.roles.get_role_by_id", AsyncMock(return_value=None)):
            res = client.get(f"/v1/admin/roles/{role_id}")
    ...
    def test_get_role_by_id_database_error(self, client):
        ...
        role_id = str(uuid.uuid4())
        with patch("apps.user_service.app.api.admin_management.roles.get_role_by_id", AsyncMock(side_effect=Exception("Database error"))), \
             patch("apps.user_service.app.api.admin_management.roles.get_role_permissions", AsyncMock(side_effect=Exception("Database error"))):
            # Since there's no error handling decorator, the exception will be raised directly
            with pytest.raises(Exception, match="Database error"):
...
class TestCreateRole:
    ...
    def test_create_role_duplicate_name(self, client):
        ...
        payload = {"name": "Admin", "description": "", "role_type": "custom", "permission_ids": []}
        with patch("apps.user_service.app.api.admin_management.roles.check_role_name_unique", AsyncMock(return_value=False)):
            res = client.post("/v1/admin/roles", json=payload)
            assert res.status_code == 400
    ...
    def test_create_role_invalid_data(self, client):
        ...
        payload = {"name": "", "description": "", "role_type": "invalid", "permission_ids": []}
        res = client.post("/v1/admin/roles", json=payload)
    ...
    def test_create_role_invalid_permission_ids(self, client):
        ...
        payload = {"name": "TestRole", "description": "", "role_type": "custom", "permission_ids": ["invalid-uuid"]}
        res = client.post("/v1/admin/roles", json=payload)
        assert res.status_code == 400
...
class TestUpdateRole:
    ...
    def test_update_role_not_found(self, client):
        ...
        role_id = str(uuid.uuid4())
        payload = {"name": "Updated", "description": "Updated"}
        with patch("apps.user_service.app.api.admin_management.roles.check_role_exists", AsyncMock(return_value=False)):
            res = client.put(f"/v1/admin/roles/{role_id}", json=payload)
            assert res.status_code == 404
            assert "Role not found" in res.json()["detail"]
    ...
    def test_update_role_duplicate_name(self, client):
        ...
        with patch("apps.user_service.app.api.admin_management.roles.check_role_exists", AsyncMock(return_value=True)), \
             patch("apps.user_service.app.api.admin_management.roles.get_role_by_id", AsyncMock(return_value={"id": role_id, "name": "Old", "description": "", "is_default": False})), \
             patch("apps.user_service.app.api.admin_management.roles.check_role_name_unique", AsyncMock(return_value=False)):
            res = client.put(f"/v1/admin/roles/{role_id}", json=payload)
            assert res.status_code == 400
            assert "Role name already exists" in res.json()["detail"]
    ...
    def test_update_role_database_error(self, client):
        ...
        role_id = str(uuid.uuid4())
        payload = {"name": "Updated", "description": "Updated"}
        with patch("apps.user_service.app.api.admin_management.roles.check_role_exists", AsyncMock(side_effect=Exception("Database error"))), \
             patch("apps.user_service.app.api.admin_management.roles.get_role_by_id", AsyncMock(side_effect=Exception("Database error"))):
            # Since there's no error handling decorator, the exception will be raised directly
            with pytest.raises(Exception, match="Database error"):
    ...
    def test_update_role_with_is_default(self, client):
        ...
        with patch("apps.user_service.app.api.admin_management.roles.check_role_exists", AsyncMock(return_value=True)), \
             ...
             patch("apps.user_service.app.api.admin_management.roles.update_role", AsyncMock(return_value=True)):
            res = client.put(f"/v1/admin/roles/{role_id}", json=payload)
            assert res.status_code == 200
            assert "Role updated successfully" in res.json()["message"]
...
class TestDeleteRole:
    ...
    def test_delete_role_default_role(self, client):
        ...
        with patch("apps.user_service.app.api.admin_management.roles.check_role_exists", AsyncMock(return_value=True)), \
             patch("apps.user_service.app.api.admin_management.roles.get_role_by_id", AsyncMock(return_value={
                 "id": role_id, "name": "Admin", "description": "", "is_default": True
             })), \
             patch("apps.user_service.app.api.admin_management.roles.check_role_usage", AsyncMock(return_value=0)), \
             patch("apps.user_service.app.api.admin_management.roles.get_role_permissions", AsyncMock(return_value=[])), \
             patch("apps.user_service.app.api.admin_management.roles.delete_role", AsyncMock(return_value=True)):
    ...
    def test_delete_role_database_error(self, client):
        ...
        with patch("apps.user_service.app.api.admin_management.roles.check_role_exists", AsyncMock(side_effect=Exception("Database error"))), \
             patch("apps.user_service.app.api.admin_management.roles.get_role_by_id", AsyncMock(side_effect=Exception("Database error"))), \
             patch("apps.user_service.app.api.admin_management.roles.check_role_usage", AsyncMock(side_effect=Exception("Database error"))):
            # Since there's no error handling decorator, the exception will be raised directly
            with pytest.raises(Exception, match="Database error"):
    ...
    def test_delete_role_in_use(self, client):
        ...
        with patch("apps.user_service.app.api.admin_management.roles.check_role_exists", AsyncMock(return_value=True)), \
             ...
             patch("apps.user_service.app.api.admin_management.roles.check_role_usage", AsyncMock(return_value=3)):  # 3 members using this role
            res = client.delete(f"/v1/admin/roles/{role_id}")
            assert res.status_code == 409
            assert "Cannot delete role. It is currently assigned to 3 organization member(s)" in res.json()["detail"]
    ...
    def test_delete_role_not_found_after_check(self, client):
        ...
        with patch("apps.user_service.app.api.admin_management.roles.check_role_exists", AsyncMock(return_value=True)), \
             ...
             patch("apps.user_service.app.api.admin_management.roles.check_role_usage", AsyncMock(return_value=0)), \
             patch("apps.user_service.app.api.admin_management.roles.delete_role", AsyncMock(return_value=False)):  # Role not found during deletion
            res = client.delete(f"/v1/admin/roles/{role_id}")
            assert res.status_code == 404
...
class TestPermissionDenied:
    ...
    def test_create_role_permission_denied(self, app_without_permission_override):
        ...
        with patch("apps.user_service.app.api.admin_management.roles.check_permissions", AsyncMock(side_effect=HTTPException(status_code=403, detail="Insufficient permissions"))), \
             patch("apps.user_service.app.api.admin_management.roles.check_permissions_exist", AsyncMock(return_value=True)), \
             patch("apps.user_service.app.api.admin_management.roles.check_role_name_unique", AsyncMock(return_value=True)), \
             patch("apps.user_service.app.api.admin_management.roles.create_role", AsyncMock(return_value={"id": str(uuid.uuid4()), "created_at": datetime.now()})):
            res = client.post("/v1/admin/roles", json=payload)
            assert res.status_code == 403
            assert "Insufficient permissions" in res.json()["detail"]
    ...
    def test_delete_role_permission_denied(self, app_without_permission_override):
        ...
        with patch("apps.user_service.app.api.admin_management.roles.check_permissions", AsyncMock(side_effect=HTTPException(status_code=403, detail="Insufficient permissions"))), \
             patch("apps.user_service.app.api.admin_management.roles.check_role_exists", AsyncMock(return_value=True)), \
             patch("apps.user_service.app.api.admin_management.roles.get_role_by_id", AsyncMock(return_value={"id": role_id, "name": "TestRole", "description": "", "is_default": False})), \
             patch("apps.user_service.app.api.admin_management.roles.check_role_usage", AsyncMock(return_value=0)):
            res = client.delete(f"/v1/admin/roles/{role_id}")
            assert res.status_code == 403
            assert "Insufficient permissions" in res.json()["detail"]
