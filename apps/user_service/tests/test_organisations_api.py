"""Test cases for organisations API endpoints.

Tests the organisations API endpoints in apps/user_service/app/api/admin_management/organisation
"""
...
def _build_org_record(**overrides):
    ...
    # Extract plan_type and max_users before updating record
    # (they belong in subscription, not top-level)
    ...
    if subscription_override:
        ...
        if plan_type is not None:
            merged["plan_type"] = plan_type.value if isinstance(plan_type, PlanType) else plan_type
...
@pytest.fixture
def app():
    """Create FastAPI app with organisation router for testing."""
    from fastapi import FastAPI

    from apps.user_service.app.api.organisation import router as org_router
...
def inject_legacy_plan_and_max_users(monkeypatch):
    """Shim missing attributes introduced by the newer schema.

    This allows the existing API logic (which still expects top-level
    plan_type/max_users) to keep working in tests.
    """
    from apps.user_service.app.api import organisation as org_module
    ...
    def body_getattr(self, item):
        ...
        if original_body_getattr is not None:
    ...
    def company_getattr(self, item):
        ...
        if original_company_getattr is not None:
...
class TestOrganisationList:
    ...
    def test_organisations_list_success(self, client):
        ...
        with (
            patch(
                ("apps.user_service.app.api.organisation.get_list_of_organisations"),
                AsyncMock(return_value=mock_organisations),
            ),
            patch(
                ("apps.user_service.app.api.organisation.get_organisations_count"),
                AsyncMock(return_value=2),
            ),
            patch(
                ("apps.user_service.app.api.organisation.extract_user_context"),
                AsyncMock(
    ...
    def test_organisations_list_with_filters(self, client):
        ...
        with (
            ...
            response = client.get(
                "/v1/admin/organisation/list?name=Test&status=active&page=1&page_size=10"
            )
    ...
    def test_orgs_list_count_unexpected_type(self, client):
        ...
        with (
            ...
            patch(
                ("apps.user_service.app.api.organisation.get_organisations_count"),
                AsyncMock(return_value="unexpected_type"),
            ),
            patch(
                ("apps.user_service.app.api.organisation.extract_user_context"),
                AsyncMock(
            ...
            patch(
                ("apps.user_service.app.api.organisation._process_organisations_data")
            ) as mock_process,
        ):
            ...
            mock_process.return_value = (
                formatted_orgs,
                0,
            )  # Simulate the else branch returning 0
...
class TestOrganisationDetails:
    ...
    def test_organisation_details_success(self, client):
        ...
        with patch(
            ("apps.user_service.app.api.organisation.get_organisation_details_by_id"),
            AsyncMock(return_value=mock_organisation),
        ):
...
class TestCreateOrganisation:
    ...
    def test_create_organisation_success(self, client):
        ...
        request_data = {
            ...
            "company_data": {
                ...
                "subscription": {"plan_type": "starter", "max_users": 10},
            },
            "plan_type": "starter",
        }
        ...
        with (
            patch(
                ("apps.user_service.app.api.organisation.create_organisation_with_super_admin"),
    ...
    def test_create_organisation_missing_user_id(self, client):
        ...
        with patch(
            ("apps.user_service.app.api.organisation.extract_user_context"),
            AsyncMock(
                return_value=MagicMock(user_id=None, email="test@example.com", organization_id=None)
            ),
        ):
            response = client.post("/v1/admin/organisation/", json=request_data)
    ...
    def test_create_organisation_slug_conflict(self, client):
        ...
        with (
            patch(
                ("apps.user_service.app.api.organisation.check_organisation_slug_unique"),
    ...
    def test_create_organisation_database_connection_error(self, client):
        ...
        with (
            ...
            patch(
                "apps.user_service.app.api.organisation.create_organisation_with_super_admin",
                AsyncMock(side_effect=ConnectionError("Database connection failed")),
            ),
            patch(
                ("apps.user_service.app.api.organisation.extract_user_context"),
                AsyncMock(
    ...
    def test_create_organisation_database_timeout_error(self, client):
        ...
        with (
            ...
            patch(
                "apps.user_service.app.api.organisation.create_organisation_with_super_admin",
                AsyncMock(side_effect=TimeoutError("Database timeout")),
            ),
            patch(
                ("apps.user_service.app.api.organisation.extract_user_context"),
                AsyncMock(
    ...
    def test_create_organisation_database_value_error(self, client):
        ...
        with (
            ...
            patch(
                ("apps.user_service.app.api.organisation.create_organisation_with_super_admin"),
                AsyncMock(side_effect=ValueError("Invalid data format")),
            ),
            patch(
                ("apps.user_service.app.api.organisation.extract_user_context"),
                AsyncMock(
    ...
    def test_create_organisation_personal_account_type(self, client):
        ...
        mock_result = {
            "organization": {
                "organization_id": str(uuid.uuid4()),
                # Should use first_name + last_name for personal
                "name": "John Doe",
        ...
        with (
            ...
            patch(
                ("apps.user_service.app.api.organisation._determine_organization_name")
            ) as mock_determine_name,
        ):
...
class TestUpdateOrganisation:
    ...
    def test_update_organisation_success(self, client):
        ...
        with (
            patch(
                ("apps.user_service.app.api.organisation.get_organisation_details_by_id"),
                AsyncMock(return_value=mock_organisation_data),
            ),
            patch(
                ("apps.user_service.app.api.organisation.update_organisation_details"),
                AsyncMock(return_value=mock_result),
            ),
        ):
    ...
    def test_update_organisation_not_found(self, client):
        ...
        organisation_id = str(uuid.uuid4())
        request_data = {"name": "Updated Organization", "domain": "updated.com"}
    ...
    def test_update_organisation_invalid_uuid(self, client):
        ...
        invalid_id = "invalid-uuid"
        request_data = {"name": "Updated Organization"}
    ...
    def test_update_organisation_permission_denied(self, client):
        ...
        with patch(
            ("apps.user_service.app.api.organisation.check_permissions"),
            AsyncMock(
...
class TestDeleteOrganisation:
    ...
    def test_delete_organisation_success(self, client):
        ...
        with (
            ...
            patch(
                ("apps.user_service.app.api.organisation.delete_organisation"),
                AsyncMock(return_value=True),
            ),
        ):
