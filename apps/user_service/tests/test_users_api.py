...
def _build_mock_supabase_user(metadata_override=None, identities_override=None):
    ...
    identities = identities_override if identities_override is not None else [default_identity]
...
def test_update_user_email_success(client):
    ...
    with (
        ...
        patch(
            (
                "apps.user_service.app.api.admin_management.users."
                "update_user.update_supabase_user_email"
            ),
...
def test_update_user_email_duplicate_email(client):
    ...
    with (
        ...
        patch(
            ...
            AsyncMock(side_effect=HTTPException(status_code=400, detail="Email already exists")),
        ),
    ):
        res = client.put(f"/v1/admin/users/{user_id}/email", json=payload)
...
def test_ban_user_self_ban(client):
    ...
    mock_check_permissions_async = AsyncMock(
