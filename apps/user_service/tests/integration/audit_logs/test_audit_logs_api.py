...
def _patch_audit_list_auth(monkeypatch, *, can_view_system: bool = True, user_type: str = "admin"):
    """Patch auth dependencies used by GET /audit-logs."""
...
async def test_get_audit_logs_with_filters(monkeypatch, client):
    ...
    res = await client.get(
        "/v1/audit-logs"
        f"?page=1&page_size=10&project_id={PROJECT_ID}"
        "&action_type=UPDATE"
        "&category=CONTACT"
        "&risk_level=medium"
...
async def test_get_audit_logs_accepts_each_action_type(monkeypatch, client, action_type):
    ...
    res = await client.get(f"/v1/audit-logs?action_type={action_type}&project_id={PROJECT_ID}")
    body = assert_success(res, 200)
...
@pytest.mark.asyncio
async def test_get_audit_logs_rejects_forbidden_project_access(monkeypatch, client):
    """Project access failure returns 403 before listing audit logs."""
    ...
    res = await client.get(f"/v1/audit-logs?page=1&page_size=10&project_id={PROJECT_ID}")
...
async def test_get_audit_logs_rejects_invalid_action_type(client):
    ...
    res = await client.get(f"/v1/audit-logs?action_type=READ&project_id={PROJECT_ID}")
...
async def test_get_audit_logs_rejects_invalid_risk_level(client):
    ...
    res = await client.get(f"/v1/audit-logs?risk_level=critical&project_id={PROJECT_ID}")
...
async def test_get_audit_logs_rejects_inverted_date_range(monkeypatch, client):
    ...
    res = await client.get(
        f"/v1/audit-logs?start_date=2026-02-01&end_date=2026-01-01&project_id={PROJECT_ID}"
    )
...
async def test_get_audit_logs_scopes_user_without_system_permission(monkeypatch, client):
    ...
    captured: dict = {}
    _patch_audit_list_auth(monkeypatch, can_view_system=False, user_type="member")
    ...
    res = await client.get(
        f"/v1/audit-logs?user_id=other-user&action_type=DELETE&project_id={PROJECT_ID}"
    )
